    ...
    <>
      ...
      <div className={`overflow-hidden border-b transition-colors duration-300 ${isScrolled ? 'border-white/10 bg-[#07080A]' : 'border-[#07080A]/10 bg-white'}`}>
        <div className="mx-auto max-w-7xl px-4 py-2">
          <div className={`animate-ticker flex items-center gap-6 whitespace-nowrap text-xs font-bold uppercase tracking-widest md:gap-8 md:text-sm ${isScrolled ? 'text-white/70' : 'text-[#1A3A6B]/70'}`}>
      ...
      {/* Main Navigation Bar */}
      <nav className={`sticky top-0 z-50 border-b transition-all duration-300 ${isScrolled ? 'border-white/10 bg-[#07080A]/95 text-white backdrop-blur-lg' : 'border-[#07080A]/10 bg-white/90 text-[#1A3A6B] backdrop-blur-md'}`}>
          ...
          <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 py-4">
            ...
            <div className="hidden min-w-0 items-center justify-center gap-6 lg:flex xl:gap-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`whitespace-nowrap text-sm font-semibold uppercase tracking-wider transition-colors hover:text-[#C9A84C] ${isScrolled ? 'text-white/70' : 'text-[#1A3A6B]/70'}`}
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="flex items-center gap-3 justify-self-end">
              <a
                ...
                className={`hidden rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-[0.22em] transition-colors md:inline-flex ${isScrolled ? 'border-white/10 text-white/80 hover:border-white/25' : 'border-[#1A3A6B]/10 text-[#1A3A6B] hover:border-[#1A3A6B]/25'}`}
              >
                +254 708 157 713
              </a>
              <Link
                href="/contact"
                className={`hidden items-center gap-2 rounded-full px-5 py-2 text-sm font-bold uppercase tracking-[0.22em] transition-all hover:scale-105 sm:inline-flex ${isScrolled ? 'bg-white text-[#1A3A6B] shadow-[0_12px_30px_rgba(255,255,255,0.08)]' : 'bg-[#1A3A6B] text-white shadow-[0_12px_30px_rgba(7,8,10,0.08)]'}`}
              >
                Book Service
              </Link>
